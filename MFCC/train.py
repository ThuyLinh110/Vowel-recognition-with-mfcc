...
# danh dau toa do bien Speech/Silence
...
def main(n_clusters):
    ...
    for i in range(len(file_name)):
        ...
        for j in range(len(folder_name)):
            avg.append(find_feature_vector('../NguyenAmHuanLuyen-16k/'+folder_name[j]+'/'+file_name[i]))
        feature_av.append(np.asarray(avg))

    feature_av = np.asarray(feature_av)
    kmean=[]
    for i in range(len(feature_av)):
        kmeans = KMeans(n_clusters=n_clusters).fit(feature_av[i])
        for j in kmeans.cluster_centers_ :
            kmean.append(j)
    kmean=np.asarray(kmean)
    np.savetxt('k_center.txt',kmean)
